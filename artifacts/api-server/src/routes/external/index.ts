 ...
 *
 * Authentication:
 *   Include header:  X-API-Key: rsdway_<your-key>
 *
...
async function resolveApiKey(req: Request, res: Response): Promise<KeyInfo | null> {
  ...
  if (!apiKey || typeof apiKey !== "string") {
    res.status(401).json({ success: false, error: "Missing X-API-Key header. Include your API key in the request header: X-API-Key: rsdway_..." });
    return null;
  }
...
router.post("/external/keys", requireAuth, async (req, res): Promise<void> => {
  ...
  const keyValue = `rsdway_${crypto.randomBytes(28).toString("hex")}`;
  const [row] = await db
    .insert(apiKeysTable)
